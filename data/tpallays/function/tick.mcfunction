...
execute as @e[type=allay,tag=!tpallays_named] at @s unless data entity @s Brain.memories."minecraft:liked_noteblock" unless block ~ ~ ~ #tpallays:trap unless block ~ ~-1 ~ #tpallays:trap run function tpallays:tp_to_player with entity @s Brain.memories."minecraft:liked_player"

#to liked noteblock
execute as @e[type=allay,tag=!tpallays_named,nbt={Brain:{memories:{"minecraft:liked_noteblock_cooldown_ticks":{value:0}}}}] at @s unless block ~ ~ ~ #tpallays:trap unless block ~ ~-1 ~ #tpallays:trap unless data entity @s CustomName run function tpallays:resetallaypos
